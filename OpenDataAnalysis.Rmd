...
dataforsh=read.table("MidResponses.csv", header=F, row.names = NULL ,  sep= ",")
...
cb1 <- cb1 + geom_hline(yintercept=0.5, linetype="dashed") + theme_classic()
cb1 <- cb1 + ylim(c(0,1))
...
cb2 <- cb2 + ylim(c(0,1)) + theme_classic()
...
ap5 <- ggplot(combineditemdata,aes(x="", y=combineditemdata$propmedhigh))+
  ...
  ggtitle('')
ap5 <- ap5 + geom_hline(yintercept=0.5, linetype="dashed") + theme_classic()
ap5 <- ap5 + ylim(c(0, 1))
...
ap6 <- ggplot(combineditemdata,aes(x="", y=combineditemdata$driftrate))+
  ...
  ggtitle('')
ap6 <- ap6 + geom_hline(yintercept=0, linetype="dashed") + theme_classic()
...
OCrange<-range(combineditemdata$OCIR-18)
OCIR <-c(mean(combineditemdata$OCIR-18),sd(combineditemdata$OCIR-18), OCrange[1], OCrange[2])
...
splithpmid <- splitHalf(dataforsh,raw=T,brute=FALSE,n.sample=100000,covar=FALSE,check.keys=TRUE, key=NULL,ci=.05,use="pairwise")
splithpmid
...
ap7 <- ap7 + geom_hline(yintercept=0, linetype="dashed")+ theme_classic()
ap7 <- ap7 + xlim(c(0,60))
...
ap8 <- ggplot(combineditemdata, aes(x=combineditemdata$BDI-20, y=combineditemdata$propmedhigh)) +
  ...
  ylab('p(mid as high)')+guides(fill = FALSE, col = FALSE)+
  xlab('BDI')+guides(fill = FALSE, col = FALSE)
ap8 <- ap8 + geom_hline(yintercept=0.5, linetype="dashed")+ theme_classic()
ap8 <- ap8 + xlim(c(0,60))
...
ap9 <- ap9 + geom_hline(yintercept=0.5, linetype="dashed")+ theme_classic()
ap9 <- ap9 + ylim(c(0,1))
...
```{r message=FALSE, warning=FALSE, echo=FALSE}
affbiasbignaomit <- combineditemdata[complete.cases(combineditemdata$propmedhigh),]

symptomdata<-na.omit(data.frame(affbiasbignaomit$BDI-20, affbiasbignaomit$Ravens, affbiasbignaomit$Age))
modsymp = Mclust(symptomdata, initialization=list(hcPairs = randomPairs(symptomdata, seed=3)), verbose = F)
...
clg1 <- ggplot(sympclustgroups,aes(x=reorder(group, (1-Pmid)),y=Pmid,fill=group,col=group))+
  ...
  geom_boxplot(outlier.shape = NA, alpha = 0, width = .1, colour = "BLACK")+
  ylab('')+scale_color_manual(values = c("gray", "gray", hcsympcol[2],  hcsympcol[1], "gray"))+scale_fill_manual(values = c("gray", "gray", hcsympcol[2],  hcsympcol[1], "gray"))+guides(fill = FALSE, col = FALSE)+
  ggtitle('Latent Classes')
clg1 <- clg1 + geom_hline(yintercept=0.5, linetype="dashed")+ theme_classic()
...
clg2 <- ggplot(sympclustgroups,aes(x=reorder(group, (1-Pmid)),y=Age,fill=group,col=group))+
  ...
  geom_point(position=position_jitter(width = .05),size = 1, alpha = 0.4)+
  geom_boxplot(outlier.shape = NA, alpha = 0, width = .1, colour = "BLACK")+
  ylab('Age')+scale_color_manual(values = c("gray", "gray", hcsympcol[2],  hcsympcol[1], "gray"))+scale_fill_manual(values = c("gray", "gray", hcsympcol[2],  hcsympcol[1], "gray"))+guides(fill = FALSE, col = FALSE)+
...
clg3 <- ggplot(sympclustgroups,aes(x=reorder(group, (1-Pmid)),y=BDI,fill=group,col=group))+
  ...
  geom_point(position=position_jitter(width = .05),size = 1, alpha = 0.4)+
  geom_boxplot(outlier.shape = NA, alpha = 0, width = .1, colour = "BLACK")+
  ylab('BDI')+scale_color_manual(values = c("gray", "gray", hcsympcol[2],  hcsympcol[1], "gray"))+scale_fill_manual(values = c("gray", "gray", hcsympcol[2],  hcsympcol[1], "gray"))+guides(fill = FALSE, col = FALSE)+
  ggtitle('')+ theme_classic()
...
clg4 <- ggplot(sympclustgroups,aes(x=reorder(group, (1-Pmid)),y=Ravens,fill=group,col=group))+
  ...
  geom_point(position=position_jitter(width = .05),size = 1, alpha = 0.4)+
  geom_boxplot(outlier.shape = NA, alpha = 0, width = .1, colour = "BLACK")+
  ylab('Ravens')+scale_color_manual(values = c("gray", "gray", hcsympcol[2],  hcsympcol[1], "gray"))+scale_fill_manual(values = c("gray", "gray", hcsympcol[2],  hcsympcol[1], "gray"))+guides(fill = FALSE, col = FALSE)+
  ggtitle('')+ theme_classic()

cl_all_plot <- plot_grid(ap9, clg1, clg3, clg4, clg2, labels="AUTO", nrow=3)
cl_all_plot
graph2ppt(file="C:/Users/orobinson/OneDrive - University College London/OneDrive_Oli/Trainees/Lucie/Paper/AnalysisMarkdown/GroupRep.pptx", width=10, height=10)
ggsave('C:/Users/orobinson/OneDrive - University College London/OneDrive_Oli/Trainees/Lucie/Paper/AnalysisMarkdown/GroupRep.pdf', width = 10, height = 10)
...
clustgroups <- data.frame(class=modsymp$classification, 
                          ...
                          Age=affbiasbignaomit$Age,
                          Ravens=affbiasbignaomit$Ravens)
Patient=subset(clustgroups, class ==3)
Healthy=subset(clustgroups, class ==4)
...
print(paste("The effect size of the Human group difference on bias is d=", (round(cohensD(Pmid ~ group, data =cluGroupBDI),digits=2))))
...
###test how frequent latent factor soln is -commented out because it takes ages to run!

###this runs the latent mixture model 1000 times with random starts (based on shuffling the data order)
#countG =NA
#for (i in 1:1000) {
#modsymp = Mclust(symptomdata, initialization=list(hcPairs = randomPairs(symptomdata)), verbose = F)
#countG[i]=modsymp$G
#}
#ggplot(data.frame(countG), aes(x=countG)) + geom_histogram(binwidth=1, color="darkblue", fill="lightblue")

#length(which(countG == 4))  #answer = 898 (90%) - so should report 4 (not 5) factor solution
#length(which(countG == 5)) #answer = 61 (6%)
...
* Family_Schizophrenia (categorical): do you have a family history of schizophrenia?
* Family_Bipolar (categorical): do you have a family history of bipolar disorder?
...
* Neurological_Disorder (categorical): do you currently have a neurological disorder?
* TakeDrugs (categorical): do you take recreational drugs?
* DrinkAlcohol (categorical): do you drink alcohol?
* Learning_Disability (categorical): do you have a learning disability?
...
STAI (r=-0.08, p=0.018), SZ (r=-0.10, p<0.001) and OCD (r=-0.09,p=0.005) predict p(mid as high) they just do not do so over and above the effect of BDI (r=-0.13, p<0.001).
...
Excluding individuals (N=36) with RTs less than 2 sd below the mean for reacting to known low value outcome has no impact on the overall inference about predictors.